...
let foldersArray = ['docs', 'videos']; // 'videos', 'files' ...
            ...
            if (items) {
                ...
                for (var i = 0; i < items.length; i++) {
                    ...
                    if (c_fontname != fontname) {
                        fs.appendFile(src_folder + '/scss/base/fonts.scss', '@include font("' + fontname + '", "' + fontname + '", "400", "normal");\r\n', cb);
                    }
                    c_fontname = fontname;
                }
            }
...
function jsBuild() {
    let appPath = path.build.js + 'script.min.js';
            ...
            'append': {
                ...
                'to': [
                    ...
                    ['image', '%TS%'],
                    {
                        ...
                        'value': '%DT%',
                        'cover': 1,
                        'files': ['script.min.js', 'vendors.min.js'] // Array [{String|Regex}] of explicit files to append to
                    }
                ]
            },
            'output': {
...
let build = gulp.parallel(htmlBuild, cssBuild, jsBuild, images);
